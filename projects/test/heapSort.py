...
class HeapSort(Scene):
    def construct(self):
        ...
        color_dict = {"最后一个非叶子结点": GOLD, "从下至上": ORANGE, "自上而下": PURPLE}
        ...
        "从下标为0的元素7开始调整堆,记录tmp=7,指针为index=0。从下标为1的左子点11开始,子节点11大于tmp=7,将子节点的值赋予于指针所 指的元素:data[0] =11,并将子节点的下标赋值给指针index=1。将7赋给指针index=1所指的位置:data[1] =7。此次调整完成。",
        ...
        for i in range(length//2-1, -1, -1):
            text = TextMobject(descriptions[descriptions_index], alignment="\\raggedright").scale(0.4)
            build_groups.add(text)
            build_groups.arrange_submobjects(DOWN, aligned_edge=LEFT, buff=.2)
            build_groups.next_to(step_one_description, DOWN)
            self.play(Write(build_groups), run_time=1)
            self.adjust_heap(data, trees, array_groups, i, length)
            descriptions_index += 1
        self.play(Uncreate(build_groups))
        # 初始堆完成
        step_two_description = TextMobject(steps_description[1], alignment="\\raggedright").scale(0.5)
        ...
        for j in range(length-1, -1, -1):
            # 文字描述
            step_swap_description = TextMobject(steps_description[2], alignment="\\raggedright").scale(0.5)
    ...
    def adjust_heap(self, data, trees, array_groups, index, len):
        ...
        k = 2 * index + 1
        while k < len:
        #for k in range(2*index + 1, len, 2*index + 1):
            ...
            if data[k] > tmp:
                ...
                #print("index=", index, array_groups.submobjects[index].submobjects[1].get_value())
                self.play(ShowCreation(array_groups.submobjects[index].submobjects[1].move_to(array_groups.submobjects[index].submobjects[0].get_center())))
                # trees
                trees.elements[index].submobjects[1] = trees.elements[k].submobjects[1]
                self.play(ShowCreation(trees.elements[index].submobjects[1].move_to(trees.elements[index].submobjects[0].get_center())))
                #
                index = k
            else:
                break
        ...
        self.play(ShowCreation(array_groups.submobjects[begin].submobjects[1]),
        ShowCreation(array_groups.submobjects[index].submobjects[1].move_to(array_groups.submobjects[index].submobjects[0].get_center())))
        # trees
    ...
    def heap_description(self):
        ...
        "当且满足如下关系时,称之为堆。",tex_to_color_map=heap_color_dict).scale(0.5)
        ...
        heap_illustrator_text.next_to([-4, -5.5, 0])
        #
        array_groups = Arrays(data=data, positions=[2, -1, 0]).get_arrays()
